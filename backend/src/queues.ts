...
export const messageQueue = new Queue("MessageQueue", connection, {
  limiter: {
    max: limiterMax as unknown as number,
    duration: limiterDuration as unknown as number
  }
});
...
async function handleSendMessage(job: any) {
  try {
    ...
    // loga objeto para evitar erro de tipagem do logger
    logger.error({ errMsg: String((e as Error)?.message ?? e) } as any, "MessageQueue -> SendMessage: error");
    throw e;
  }
}
...
async function handleVerifySchedules() {
  try {
    ...
    Sentry.captureException(e);
    logger.error({ errMsg: String((e as Error)?.message ?? e) } as any, "SendScheduledMessage -> Verify: error");
    throw e;
  }
}
...
async function handleSendScheduledMessage(job: any) {
  ...
  try {
    ...
    logger.error({ errMsg: String((e as Error)?.message ?? e) } as any, "SendScheduledMessage -> SendMessage: error");
    throw e;
  }
}
...
async function handleRatingsTimeout() {
  ...
  const ratingThresholds: Record<number, Date> = {};
...
async function handleEveryMinute() {
  ...
  try {
    ...
    logger.error({ errMsg: String((e as Error)?.message ?? e) } as any, `handleEveryMinute: error received`);
  }
}
